...
4. **Variações no Payload da API (Notícias Vazias):** Durante a nossa inspeção dos artefatos coletados na base (`raw_data/*.json`), notamos que a extração para alguns domínios (apresentando como exemplos os domínios "World", "Tech" e "Finance") entregou a chave nativa `"news"` como um vetor vazio `[]`. Nossa rastreabilidade apontou que isso não é um bug no motor de Web Scraping do Playwright e não ocorreu erros de timeout, indicando que a limitação no fornecimento desta categoria das notícias é restritiva das próprias views/API originais da plataforma simulador.
5. **Dados de Mercado Financeiro (Oportunidades Futuras):** Mesmo os endpoints contendo chaves `"news"` vazias trouxeram rica carga massiva em formato estruturado na chave `"markets"`. São informações que englobam os *tickers* (Ações, Criptomoedas, Tokens, etc.), além do seu preço monetário exato e de mercado. Isso abre oportunidades para evoluirmos nossos processos integrando futuramente um relatório secundário contendo a precificação pura desses ativos (e.g. `market_tickers.csv`), sendo útil para algoritmos de AI que atuam diretamente no trade em vez de depender apenas do aspecto linguístico da NLP.
